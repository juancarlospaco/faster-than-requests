...
extern tyProc__ln4kdL5W9bbX4a1xl8nnVXQ Dl_15665614_;
extern NIM_THREADVAR NIM_BOOL nimInErrorMode__759bT87luu8XGcbkw13FUjA;
extern tyProc__ln4kdL5W9bbX4a1xl8nnVXQ Dl_15710677_;
extern tyProc__Vff1ipOwDPDGknaRJAdXjA Dl_15665696_;
extern tyProc__kxUw9cdxJbPtv0iPyoSbDBQ Dl_15665719_;
extern tyProc__wCuLyuC5VitcgJKGCqJQRw Dl_15666026_;
extern tyProc__5zMKCPIiPHTYY9cjyzsHlmQ Dl_15666020_;
extern tyProc__e47RoMUolEGMiUlfiZt9abQ Dl_15665619_;
extern int osInvalidSocket__voz9aUXu8jtRbvGZZJHNE8w;
extern tyProc__Rqjya8uauy9aPcyfGamaSrg Dl_15665616_;
extern tyProc__kxUw9cdxJbPtv0iPyoSbDBQ Dl_15665692_;
extern NIM_THREADVAR Exception* currException__9bVPeDJlYTi9bQApZpfH8wjg;
extern NIM_THREADVAR Exception* currException__9bVPeDJlYTi9bQApZpfH8wjg;
extern NIM_THREADVAR Exception* currException__9bVPeDJlYTi9bQApZpfH8wjg;
extern tyProc__Vff1ipOwDPDGknaRJAdXjA Dl_15665706_;
extern tyProc__Vff1ipOwDPDGknaRJAdXjA Dl_15665723_;
extern tyProc__sG7rgqmtlgBaumPwwN3BfQ Dl_15665714_;
extern tyProc__Vff1ipOwDPDGknaRJAdXjA Dl_15665726_;
extern tyProc__09aW01hZ1tSPafLaWVpBcXA Dl_15665709_;
extern tyProc__Rqjya8uauy9aPcyfGamaSrg Dl_15665632_;
extern tyProc__HXibadfWHwHwyhCwpIRD2A Dl_15651252_;
extern tyProc__2PA7CxitARKs0jpUW1gQ0g Dl_15665655_;
extern tyProc__JfY9aCDiBh9bgOEieo8Vltxg Dl_15710459_;
extern tyProc__H5wL7jexPlRA7WCw9cqLm0A Dl_15665643_;
extern tyProc__tYatIIo0oRSB9bjHLep7yhQ Dl_15665664_;
extern tyProc__xjxKRHVZXDqdWXw9bunoZ3w Dl_15665668_;
extern tyProc__Vff1ipOwDPDGknaRJAdXjA Dl_15665673_;
...
{nimErr_ = nimErrorFlag();
	{
		if (!(*socket).isSsl) goto LA3_;
		{
			...
			ret = Dl_15665719_((*socket).sslHandle, ((int) (err)));
			switch (ret) {
			case ((NI) 6):
			{
...
appendString(errStr, ((NimStringDesc*) &TM__XqsrSBxhOTPgLQ9cX4MmV4A_17));
					...
					LA38_: ;
					{
						...
						NimStringDesc* T42_;
						T41_ = (NCSTRING)0;
						T41_ = Dl_15666020_(sslErr, NIM_NIL);
...
{nimErr_ = nimErrorFlag();
	{
		...
		if (!T4_) goto LA6_;
		{
			...
			LA30_: ;
			{
				...
				NI32 T38_;
				T35_ = (int)0;
				T35_ = pthread_sigmask(((int) 0), (&watchSetX60gensym101_), (&oldSetX60gensym101_));
			...
			Dl_15710677_();
			res = Dl_15665696_((*socket).sslHandle);
			{
			...
			LA43_: ;
			{
				...
				sslError = Dl_15665719_((*socket).sslHandle, res);
				{
					NIM_BOOL T50_;
					T50_ = (NIM_BOOL)0;
			...
			}
			{
				oldNimErrFin40_ = *nimErr_; *nimErr_ = NIM_FALSE;
				{
					if (!!(alreadyBlockedX60gensym101_)) goto LA76_;
					{
						...
						NI32 T83_;
						T80_ = (int)0;
						T80_ = pthread_sigmask(((int) 1), (&watchSetX60gensym101_), (&oldSetX60gensym101_));
...
{nimErr_ = nimErrorFlag();
	...
	(*socket).sslHandle = Dl_15665616_((*(*socket).sslContext).context);
	...
	LA3_: ;
	{
		int T7_;
		T7_ = (int)0;
		T7_ = Dl_15665692_((*socket).sslHandle, (*socket).fd);
...
{nimErr_ = nimErrorFlag();
	...
	case ((tyEnum_SslHandshakeType__9aZhh5f9bRDKgYB8GLASDSqg) 0):
	{
		...
		LA7_: ;
		Dl_15710677_();
		ret = Dl_15665706_((*socket).sslHandle);
	...
	case ((tyEnum_SslHandshakeType__9aZhh5f9bRDKgYB8GLASDSqg) 1):
	{
		int ret_2;
		Dl_15710677_();
		ret_2 = Dl_15665723_((*socket).sslHandle);
...
{	result = (NI)0;
	{
		...
		Dl_15710677_();
		T5_ = (int)0;
		T5_ = Dl_15665714_((*socket).sslHandle, ((NCSTRING) (data)), size);
		result = ((NI) (T5_));
		goto BeforeRet_;
	}
	LA3_: ;
...
appendString(T24_, ((NimStringDesc*) &TM__XqsrSBxhOTPgLQ9cX4MmV4A_36));
		...
		LA21_: ;
		{
			...
			LA32_: ;
			sslPending = Dl_15665726_((*socket).sslHandle);
			{
...
{	result = (NI)0;
	result = ((NI) 0);
	{
		...
		Dl_15710677_();
		T5_ = (int)0;
		T5_ = Dl_15665709_((*socket).sslHandle, buffer, ((NI) (size)));
		result = ((NI) (T5_));
		goto BeforeRet_;
	}
	LA3_: ;
...
{nimErr_ = nimErrorFlag();
	result = (int)0;
	{
		...
		if (!T3_) goto LA7_;
		T9_ = (long)0;
		T9_ = Dl_15710459_(ctx, ((int) 94), ((long) (onoff)), NIM_NIL);
		result = ((int) (T9_));
	}
	goto LA1_;
	LA7_: ;
...
appendString(T18_, keyFile);
	...
	LA15_: ;
	{
		...
		ret = Dl_15665664_(ctx, nimToCStringConv(certFile));
		{
	...
	LA21_: ;
	{
		if (!!(((keyFile ? keyFile->Sup.len : 0) == 0))) goto LA29_;
		{
			int T33_;
			T33_ = (int)0;
			T33_ = Dl_15665668_(ctx, nimToCStringConv(keyFile), ((int) 1));
...
{nimErr_ = nimErrorFlag();
	...
	}
	{
		int T9_;
		T9_ = (int)0;
		T9_ = Dl_15665655_(newCTX, nimToCStringConv(cipherList));
	...
	LA26_: ;
	Dl_15665643_(newCTX, ((NI) 0), NIM_NIL);
	{
