...
RUN pip3 install --upgrade requests==2.21.0
...
RUN nim c -d:release -d:ssl --app:lib --passL:"-s" --gc:markAndSweep --out:/tmp/faster_than_requests.so /tmp/faster_than_requests.nim
